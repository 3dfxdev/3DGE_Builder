		...
		public Plugin(string filename)
		{
			...
			if(t != null)
			{
				...
				if((thisrevision != 0) && (plug.MinimumRevision > thisrevision))
				{
					...
					General.ErrorLogger.Add(ErrorType.Error, "Could not load plugin \"" + shortfilename + "\", the Plugin is made for 3DGE Builder R" + plug.MinimumRevision + " or newer and you are running R" + thisrevision + ".");
					throw new InvalidProgramException();
				}
			}
			else
